...
const LineChartShare: Component = () => {
  ...
  const fetchChartData = async () => {
    try {
      const response = await fetch('http://127.0.0.1:8080/chartgender');
      const textData = await response.text();
      const [maleText, femaleText] = textData.match(/\d+/g);
      const maleCount = parseInt(maleText);
      const femaleCount = parseInt(femaleText);

      const data = [
        { category: "Laki-Laki", value: maleCount },
        { category: "Perempuan", value: femaleCount }
      ];
